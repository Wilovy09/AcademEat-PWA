...
async function createStore() {
  try {
    ...
    await router.push({ name: 'my-store' })
  } catch (error) {
    console.log(error)
  }
}
...
<template>
  <div
    @click="showSelectPortraitImage = !showSelectPortraitImage"
    class="flex justify-center"
  >
    <img
      :src="portraitImageBase64 || 'https://placehold.co/600x300'"
      alt="store portrait"
      class="object-cover w-full aspect-[2/1]"
    />
  </div>
  <div
    @click="showSelectStoreImage = !showSelectStoreImage"
    class="flex justify-center -mt-20"
  >
    <img
      :src="storeImageBase64 || 'https://placehold.co/400x400'"
      alt="store image"
  ...
  <form
    ...
    class="mx-4 gap-4 h-[calc(100vh-22rem)] flex flex-col justify-between"
  >
    <div>
      <label v-show="showSelectPortraitImage">
        Store portrait image
        <input
          @change="event => handleFileChange(event, 'portrait')"
      ...
      <VInput
        ...
        type="text"
        placeholder="My amazing store description"
        hasAutocomplete="off"
      />
    ...
    <button class="w-full bg-blue-500 text-white p-2 rounded-lg mb-6">
      Create store
    </button>
  </form>
</template>
